...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'product_type', 'download_file', 'file_type', 'file_size', 'can_download'
        ]
    ...
    def get_can_download(self, obj):
        """Check if user can download this product file."""
        user = self.context.get('request').user if 'request' in self.context else None
        if not user or not user.is_authenticated or obj.product_type != 'file' or not obj.download_file:
            return False
...
class UpdateProductSerializer(serializers.ModelSerializer):
    ...
    download_file = serializers.FileField(required=False, allow_null=True)

    class Meta:
        model = Product
        fields = ['title', 'slug', 'description', 'price', 'discount_price', 'main_image', 'delivery_time', 'stock', 'category', 'is_active', 'show_in_hero', 'product_type', 'download_file']
    ...
    def to_internal_value(self, data):
        ...
        # Handle download_file field
        if 'download_file' in data:
            val = data['download_file']
            if isinstance(val, str) or val is None or val == '' or val == 'null':
                data.pop('download_file')
